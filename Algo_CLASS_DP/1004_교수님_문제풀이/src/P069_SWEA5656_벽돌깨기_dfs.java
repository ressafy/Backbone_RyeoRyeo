...
public class P069_SWEA5656_벽돌깨기_dfs {
	...
	static boolean go(int [][] map,int cnt) {  // map 은 직전 구슬까지 던진 map 의 상태
		...
		for (int i = 0; i < W; i++) {
			...
			if(r == H) {  // 맞는 시작 벽돌이 없는 상태
				...
				// 제거될 벽돌 연쇄 처리
				boom(newMap,r,i);   // bfs, dfs 둘 다 구현이 가능하다.
				// 벽돌 중력 처리
...
//	}
	...
	private static void boom(int[][] newMap, int r, int c) {
		...
		if(newMap[r][c]>1) {
			que.offer(new Point(r,c,newMap[r][c]));
		}
		
		newMap[r][c] = 0;  // 방문처리 -> 제거처리
		
		while(!que.isEmpty()) {
			Point p = que.poll(); // 주변에 영향이 오는 벽돌만 온다. 1 보다 큰 값을 넣어줬으니
			
			// 벽돌의 크기 -1 만큼 주변 벽돌 
			for (int i = 0; i < 4; i++) {
				int nr = p.r;  // 처음 출발을 현재 벽돌의 위치로 잡아두고 
				int nc = p.c;
				for (int j = 1; j < p.cnt; j++) {  // 현재 방향에서 cnt - 1 벽돌 보기
					...
					if(nr >= 0 && nr < H && nc >= 0 && nc < W && newMap[nr][nc]>0) {
						if(newMap[nr][nc]>1) {
							que.offer(new Point(nr,nc,newMap[nr][nc]));
						}
						// 벽돌이 있던 자리를 0으로 변경: 빈칸으로 만들어서 방문처리
						newMap[nr][nc] = 0;  // 방문처리 -> 제거처리
					}
				}
			}
		}
